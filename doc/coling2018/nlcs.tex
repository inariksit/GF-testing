...
\NormalTok{concrete }\DataTypeTok{FoodsSpa} \KeywordTok{of} \DataTypeTok{Foods} \FunctionTok{=} \NormalTok{\{}
  ...
  \NormalTok{oper}
\NormalTok{    mkNP }\OtherTok{:} \NormalTok{(num }\OtherTok{:} \DataTypeTok{Number}\NormalTok{) }\OtherTok{->} \NormalTok{(masc,fem }\OtherTok{:} \DataTypeTok{String}\NormalTok{) }\OtherTok{->} \NormalTok{(cn }\OtherTok{:} \DataTypeTok{CN}\NormalTok{) }\OtherTok{->} \DataTypeTok{NP} \FunctionTok{=}
...
\paragraph{Enumerate functions} As we explained before, each syntactic
...
\paragraph{Enumerate arguments} Some syntactic functions are
...
% For example, when we test the predication function, we want to pick an
...
\paragraph{Enumerate contexts} The third and last enumeration we
...
\paragraph{Pruning the trees within one set of test cases} 
...
\paragraph{Pruning the trees to test the whole grammar}
...
trees we have seen so far: for example, the abstract tree for the sentence ``how far is the bar?'' in the Phrasebook is {\tt \small PQuestion (HowFar (ThePlace Bar))}, in contrast to the resource grammar tree
