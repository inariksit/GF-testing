...
\multicolumn{3}{|r}{Concrete $\rightarrow$}              &
  ...
  $\downarrow$ Abstract  & \#funs+lex & \#trees
   ...
   & ?? & ?? &  ?? & ?? & ??  & ?? \\ \hline
  %% Only 149 functions -- TODO run the whole thing for Basque
  %% & 9,729 & 9,161 &  13,733  & 9,194    & 12,121  & 10,634 \\ \hline
\end{tabular}}
\caption{Test cases for all functions in two grammars (with pruning)}
\label{results}
\end{table}
...
trees for all functions took several hours. Times for individual
functions ranged from 5 minutes to 40 minutes.
...
We were surprised by the similar amount of trees. 
In our initial experiments, without the pruning based on coercions
(explained in Section~\ref{sec:pruning_coercions}), we arrived at a
larger number of trees, and also larger differences---around 3 times
more for Basque, but only around 10\% more for Dutch and Estonian. 
On a more speculative note, there might be a bug in the Basque grammar
which eliminates some of the variation: all the distinctions are
created on lower levels, but some are ignored on the way to the start
category, thus creating a large number of coercions.

Table~\ref{results_indiv} 
shows some expected patterns: Basque and Estonian have more complex
nouns than Dutch, so \t{RelNP} needs more tests.
...
between implementations: Enache \cite{enache2010} reports a 200-time
...
intermediate non-native, and for Basque, a beginner. None of the three
...
using the resource grammar, so it was as error-ridden as the resource grammar.
% We think that the best course
...
For Estonian, we found a handful of bugs, all coming
