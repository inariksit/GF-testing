...
cases, but Basque verb morphology is much more complex, with agreement
in subject, object and indirect object.  
...
all syntactic functions in the two grammars. 
...
\multicolumn{3}{|r}{Concrete $\rightarrow$}              &
  ...
  $\downarrow$ Abstract  & \#funs+lex & \#trees
   ...
   & 21,370  & 19,825 &  13,733  & 9,194    & 10,185  & 8,773 \\ \hline
\end{tabular}
...
%    & 21,370  & 19,825 &  13,733  & 9,194    & 10,185  & 8,773 \\ \hline
% \end{tabular}
% \caption{Test cases for all functions in two grammars without pruning}
% \label{results}
% \end{table}
...
functions ranged from 5-10 minutes to an hour.
...
We were surprised to see the number of trees so similar, and even
smaller for the morphologically richer languages. 
In our initial experiments, without the pruning based on coercions
(explained in Section~\ref{sec:pruning_coercions}), we arrived at much
larger number of trees, and also much larger differences---an order of
magnitude for Basque, and 4-5 times higher for Dutch and Estonian. 
Out of the three languages, Basque is the least mature, thus it is
possible that the relatively low number is due to a bug somewhere: all
the distinctions are created on lower levels, but it does not travel all
the way to the start category, thus creating a large number of coercions.
