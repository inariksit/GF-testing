...
arguments and build new terms. 
The function \t{Pred} constructs an \t{S} from an
\t{NP} and an \t{AP}: e.g. \t{Pred (This Pizza) Good} ``this pizza is
good''. \t{Mod}~adds an \t{AP} to a \t{CN}, such as \t{Mod Italian
  Pizza} ``Italian pizza''.
...
is a table from number to string (\emph{pizza},
\emph{pizzas}), and another field \t{g}, which contains its gender
...
language-specific details, such as agreemen: when \t{Pred (This Pizza)
  Good} is linearized in Spanish, the copula must be singular
(la pizza \emph{es} buena), and the adjective must be in singular
feminine (\emph{buena}). In contrast, for an English concrete syntax,
only the number of the copula is relevant.
