...
The following test set creates all the relevant cases:
 \t{PrepNP} \{ \stackanchor{\tt on}{\tt with} \} {\t{(DetCN} \{ \stackanchor{\tt this}{\tt these} \} \t{AdjCN} \{ \stackanchor{\stackanchor{\tt small}{\tt tired}}{\stackanchor{\tt ready}{}} \} {\tt house)}

%To test the whole phenomenon, we would need one more context, where the adjective is in the predicative position `the house is \verb|_|', and one more example adjective---as an attribute, we can lump participles and invariables into one test example, but as a predicative, regular and participles behave the same and differently from invariable.
