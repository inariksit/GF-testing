...
$\downarrow$ Abstract grammar & \#funs+lex & \#relevant trees  &
...
testing a whole grammar from scratch---this gives merely a baseline
...
Phrasebook function has a bug, say ``Spaniard restaurant'' instead of
``Spanish restaurant'', the problem could be in either grammar.
In the first case, the resource grammar contains both ``Spanish'' and
``Spaniard'' in correct places, but the application grammarian has
chosen a wrong function.
In the second case, the application grammarian has used a correct
function from the resource grammar, but the resource grammar function
itself has a bug; maybe it only contains ``Spaniard'' and no
``Spanish'', or the functions are swapped.
...
3--4 minutes,
 Several other languages, such as English, Swedish, \todo{Hindi, Bulgarian, ...} also
...
For the \todo{2000?} sentences in Estonian, it took __ minutes for a
...
we are planning to improve our tool to create intentionally trees that test several functions at once. 
...
\todo{cite}, and for testing morphology, various open-source resources
