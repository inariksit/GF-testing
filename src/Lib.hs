...
bestExamples gr [] = []
bestExamples gr trees = take 1 $ map fst $ sortBy goodTest trees_lins


 where
  trees_lins = [ (tree,map snd (tabularLin gr tree)) | tree <- trees ]
  goodTest :: (Eq a) => (b,[a]) -> (b,[a]) -> Ordering
  goodTest x y = if testsAsWellAs (snd x) (snd y) then GT else LT
  --tabLins = map (snd . tabularLin gr) trees :: [[String]] -- one list for one tree
